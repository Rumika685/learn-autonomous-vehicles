...
# ====== パラメータ範囲 ======
false_pos_probs = np.linspace(0, 0.3, 7)  # 0〜0.3 を 7ステップ
...
im = plt.imshow(heatmap, origin="lower", cmap="viridis",
                extent=[false_pos_probs[0], false_pos_probs[-1], false_neg_probs[0], false_neg_probs[-1]],
                aspect="auto")
plt.colorbar(im, label="Success Rate")
plt.xlabel("False Positive Probability")
plt.ylabel("False Negative Probability")
plt.title("Success Rate Heatmap: False Positive × False Negative")
plt.show()
