...
# ====== LiDAR風センサー ======
def perceive_lidar(grid, distance=3, false_pos=0.05, false_neg=0.05):
...
def run_experiment(distance, trials=50):
    ...
    for _ in range(trials):
        grid = generate_grid()
        perceived = perceive_lidar(grid, distance=distance)
...
plt.plot(distances, results, marker="o")
plt.ylim(0,1)
plt.xlabel("Sensor Range (cells)")
